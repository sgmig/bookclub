...
9. **Open**: `ClubMeetingViewSet.create`/`update` are broken independent of permissions. `ClubMeetingSerializer` nests `location` (`LocationSerializer()`) and `discussed_books` (`ReadingListItemSerializer(many=True)`) as writable-by-default, with no `create()`/`update()` override on the serializer or the viewset — confirmed empirically that any valid `POST`/`PUT`/`PATCH` against `/clubs/api/club-meeting/` raises `AssertionError: The .create() method does not support writable nested fields by default`. Found while implementing `feature/club-membership-drf-api`; pre-existing, unrelated to that branch's permission changes. See `docs/RECOMMENDATIONS.md` §"Permissions review".
